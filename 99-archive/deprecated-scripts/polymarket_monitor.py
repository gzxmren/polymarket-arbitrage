...
# Metaculus API Key
METACULUS_API_KEY = "9b2e87896a69bf1b48fd8750a188ce54c1655307"
...
def fetch_api(endpoint: str, retries=3):
    """Fetch data from Gamma API with retry logic and proper error handling."""
    ...
    for attempt in range(retries):
        try:
            ...
            last_error = e
            print(f"URL Error for {url}: {e}, retrying... (attempt {attempt+1}/{retries})", file=sys.stderr)
            time.sleep(1)
...
def get_leaderboard():
    ...
    [修复] 2024-03-13: /users 和 /leaderboard 端点返回 403/404 错误，
    已确认这些端点不可用或需要特殊认证。暂时禁用鲸鱼数据获取，
    避免产生大量错误日志。
    
    TODO: 未来可以通过以下方式获取鲸鱼数据：
    1. 分析大额交易数据
    2. 使用官方 CLOB API（需要 API key）
    3. 从第三方数据源获取
    """
    # 这些端点已不可用，直接返回空列表
...
def has_significant_changes(current, previous):
    ...
    curr_sig = get_state_signature(current)
    prev_sig = get_state_signature(previous)
    
    # 检查关键指标是否变化
    count_keys = ['opportunity_count', 'mover_count', 'spread_count', 'whale_count', 
                  'high_volume_count', 'new_market_count']
    for key in count_keys:
        if curr_sig.get(key) != prev_sig.get(key):
            return True
    
    # 检查最佳机会是否变化（避免同一机会重复报告）
    if curr_sig.get('top_opportunity_id') != prev_sig.get('top_opportunity_id'):
        return True
    if curr_sig.get('top_mover_id') != prev_sig.get('top_mover_id'):
        return True
...
def print_analysis(timestamp, total_scanned, markets, opportunities, movers, spreads, 
    ...
    current_state = {
        ...
        'top_opportunity_id': opportunities[0]['id'] if opportunities else None,
        'top_mover_id': movers[0]['id'] if movers else None,
    }
    ...
    # 构建 Telegram 报告
    report = f"""🦐 *Polymarket 监控报告* | {timestamp}

📊 *市场概览:*
• 扫描市场: {total_scanned} 个
• 活跃市场: {len(markets)} 个
...
"""
    if opportunities:
        best = opportunities[0]
        report += f"⭐ *最佳机会:* {best['question'][:50]}\n"
        report += f"   • Pair Cost: {best['pair_cost']:.4f}\n"
        report += f"   • 预期利润: {best['profit_pct']:.2f}%\n"
    
    report += f"\n📈 *价格异动 (1h >1.5%):* {len(movers)} 个\n"
    if movers:
        for m in movers[:2]:
            report += f"• {m['question'][:40]}... | {m['price_change_1h']*100:+.1f}%\n"
    
    report += f"\n🔥 *高交易量市场:* {len(high_volume_markets)} 个\n"
    report += f"🆕 *新上市市场:* {len(new_markets)} 个\n"
    report += f"🐋 *活跃鲸鱼:* {whale_count} 个\n"

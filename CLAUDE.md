...
| 3. **设计 review** | ⭐**动代码之前**先审设计:具体失败场景、设计里的错。**派 subagent(sonnet)独立审** | 同上(追加一节,写明谁审的、改了什么) |
| 4. **代码(TDD)** | 判据**先写并先确认是红的**,再写实现 | `10-tests/` |
| 5. **代码 review** | `python-reviewer` **subagent(sonnet)**;逐条自己核实再采纳 | 提交信息里记结论 |
...
### ⭐ 两处 review 都交给 subagent(2026-09-02 用户明令)

> 「另外你的 review 步骤最好用 subagent(sonnet)去执行 review 工作」

第 3 步与第 5 步一律派 `Agent`(`model: sonnet`)独立审,**自己那一轮照做但不算数**。
理由就是本文件上面那句已经写过的话:*自己写的代码自己 review,今天已被证明无效*。
2026-09-02 尤其说明问题 —— 我自己那轮设计 review 抓到 9 条(含推翻自己选的方案),
**看起来很有效**,但"我找得到的错"与"我看不见的盲区"是两个集合,前者再多也不能证明后者是空的。

给 subagent 的提示必须自带:项目背景、要读的文件与行号、**已拍板不许再改的点**
(否则它会重提被否掉的备选)、要它找什么、以及要求它标明依据是"读出来的"还是"推的"。
⚠️ 拿回意见后**逐条自己核实**再采纳 —— 子代理同样会说错。
...
| 🆕 **全局 2.5:报数字前先答偏差方向** | `~/.claude/CLAUDE.md` | 有没有真的在**报数之前**拦下过一个偏乐观的估计?⚠️ 它让全局文件涨了 19%,而那个文件明确要求精简 ⇒ **没触发就删,不许因为"话说得对"留着** |

...
1. **锁文件残留**:`backfill_market_times.main` 用独占方式创建 `.backfill.lock`,
   靠 `finally` 删除。进程被 SIGKILL(超时被 systemd 砍)不走 `finally` ⇒ 锁留下 ⇒
   **此后每一轮都立刻 return 2、什么都不做**,而 timer 仍是 active。
